...
class KardexKitImportPreprocessPalletManager extends PalletManager {
    ...
    onInput(msg){
        ...
        const findCMP = async(jobnumber, bom) =>{
            let dimPattern = new RegExp("(?:(\d+\.?\d*)(?:[^a-zA-Z0-9_\-]+\s*[a-z]*[^a-zA-Z0-9_\-]+[a-z]?\s*)(\d+\.?\d*))", "gi");
            let p20Pattern = new RegExp("\s*[A-Oa-oQ-Zq-z]*\s*(P\s*.?\s*20)\s*", "gi");
            let hhp20Pattern = new RegExp("\s*(HH\s*.?\s*P\s*.?\s*20)\s*", "gi");
            let metricPattern = new RegExp("(?:(\d+)(?:[a-z]+\s*[a-z]*\W*[a-z]?\s*)(\d+))", "gi");
            let metricPattern2 = new RegExp("(?:\s*)(?:M)(\d+\.?\d*)(?:\s*[Xx]\s*)(\d+\.?\d*)(?:\s*)", "gi");
            let unilifterPattern = new RegExp("\W*uni(?:[- ]*)lifter\W*", "gi");
            let moldmaxPattern = new RegExp("\W*mold(?:[- ]*)max\W*");
            let liftCarriagePattern = new RegExp(".*car.*ge", "gi");
            let liftPattern = new RegExp("(^LFT)", "gi");
            let lcpPattern = new RegExp("(^LCP)", "gi");
            let srtPattern = new RegExp("(^srt)", "gi");
            let sclPattern = new RegExp("(^sci)", "gi");
            let orcPattern = new RegExp("(^orc)", "gi");
            let liftDescriptionPattern = new RegExp(".*lifter","gi");
            let insPattern = new RegExp(".*ins(\s+|ert+).*", "gi");
            let whiteSpacePattern = new RegExp("^\s*$", "gi");
            ...
            for (var ri=0; ri < bom.length; ri++){
                ...
                if (p20Match !== null && hhp20Match !== null){
                    ...
                    found = true;
                } else if (p20Match !== null && liftDescMatch !== null && spaceMatch !== null){
                    found = true;
                } else if (p20Match !== null && insMatch !== null && spaceMatch !== null){
                    found = true;
                } else if (carMatch !== null && orcMatch !== null){
                    found = true;
                ...
                if (found){
                    ...
                    while((dimMatches = dimPattern.exec(bom)) != null){
                        dimArray.push(dimMatches[0]);
                        dimCount++;
                    }

                    if (dimCount <= 2 && dimCount > 0){
                        if (dimCount == 2 && dimArray[0] < 36 && dimArray[1] < 36){
        ...
        const createBinsKit = async(data) =>{
            fs.readFile(".\assets\kit_bin_sizes.json", (err, data) => {
                if (err) throw err
        ...
        const createBinsBOM = async(jobnumber, bom) =>{
            let cmpArea = 0;
            let cmpTrayCount = 1;
            let bom_bin = {name:"", width: []};
            let bom_bins = [];
            ...
            for (var ri=0; ri < bom.length; ri++){
                //check if cmp bin
                if (bom[ri].length === 11){
                    cmpArea = cmpArea + bom[ri][10];
                    if (Math.ceil(cmpArea) > Math.ceil(maxTrayDepth*maxCmpTrayWidth)){
                        cmpArea = 0
                        cmpTrayCount++;
                        
                    }
                    bom[ri][10] = cmpBinName;
                } else {
                    bom[ri].push(stdBinName);
                }
            }

            bom_bin.name = stdBinName;
            bom_bin.width = maxStdTrayWidth;
            bom_bins.push(bom_bin);

            bom_bin.name = cmpBinName;
            bom_bin.width = 48;
            bom_bins.push(bom_bin);

            if (cmpTrayCount > 1){  
                for (var idx = 1; idx < cmpTrayCount-1; idx++){
                    bom_bin.name = cmpBinName + idx.toString();
                    bom_bin.width = 48;
                    bom_bins.push(bom_bin);
                }
                bom_bin.name = cmpBinName + idx.toString();
                bom_bin.width = Math.max(cmpArea, 24);
                bom_bins.push(bom_bin);
            }
        ...
        (async() => {
            ...
            try {
                ...
                if (new_bom === null || typeof new_bom === 'undefined'){
                    ...
                    let bom_bins = await createBinsBOM(jobnumber, new_bom);
                    let kit_bins = await createBinsKit(jobnumber, data);
                    var newMsg = {};
